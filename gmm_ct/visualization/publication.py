...
def plot_temporal_gmm_comparison(
    ...
    K,
    d,
    # timestamps: list[float] | int | None = [0.37, 0.83, 0.98],
    ...
    Parameters:
    - timestamps: Can be:
        * List/Array of float time targets in seconds (e.g. [0.37, 0.83, 0.98])
        * An integer specifying how many evenly-spaced frames to sample across `t`
    ...
    if timestamps is None:
        ...
        time_indices = np.array(
            [int(np.argmin(np.abs(t_np - t_target))) for t_target in timestamps]
        )
    ...
    if spatial_bounds is None:
        ...
        for k in range(K):
            ...
            for t_val in t:
                t_scalar = t_val.item()
                pos = x0_true + v0_true * t_scalar + 0.5 * a0_true * t_scalar**2
                all_positions.append(pos)
    ...
    for row_idx, (t_idx, t_val) in enumerate(zip(time_indices, selected_times)):
        ...
        # LEFT COLUMN: True GMM
        ax_left = fig.add_subplot(gs[row_idx, 0])

        if show_trajectories:
            plot_trajectories_single(
                ax_left, theta_true, t, K, colors, mirror=False
            )
        ...
        if row_idx == 0:
            ax_left.set_title(
                "Simulated", fontweight="bold", fontsize=title_fontsize, pad=12
            )
            from matplotlib.lines import Line2D
            from matplotlib.patches import Patch
        ...
        ax_left.set_ylabel(
            f"t = {t_val_scalar:.2f} s\nHeight",
            fontweight="bold",
            fontsize=label_fontsize,
        )
        if row_idx == n_times - 1:
            ax_left.set_xlabel(
                "Depth (m)", fontweight="bold", fontsize=label_fontsize
            )
            ax_left.tick_params(
            ...
            ax_left.tick_params(
                axis="y", which="major", labelsize=tick_fontsize
            )
            ax_left.tick_params(axis="x", which="major", labelbottom=False)
        ...
        receiver_heights_arr = np.array(
            [rcvr[1].item() for rcvr in receivers[0]]
        )
        y_min = receiver_heights_arr.min() - 0.1
        y_max = receiver_heights_arr.max() + 0.1
        ax_left.set_ylim(y_min, y_max)
        ...
        if row_idx == 0:
            ...
            ax_center.legend(
                fontsize=_FS_LEGEND, loc="best", framealpha=0.9
            )

        if row_idx == n_times - 1:
            ax_center.set_xlabel(
                "Intensity", fontweight="bold", fontsize=label_fontsize
            )
            ax_center.tick_params(
                axis="x", which="major", labelsize=tick_fontsize
            )
        else:
            ax_center.tick_params(axis="x", which="major", labelbottom=False)
        ax_center.tick_params(
            axis="y", which="major", labelleft=False, labelsize=tick_fontsize
        )
        ax_center.grid(True, alpha=0.3, linestyle="--")

        # RIGHT COLUMN: Estimated GMM
        ax_right = fig.add_subplot(gs[row_idx, 2])

        if show_trajectories:
            plot_trajectories_single(
                ax_right, theta_est_reordered, t, K, colors, mirror=True
            )
        ...
        if row_idx == 0:
            ax_right.set_title(
                f"{title}", fontweight="bold", fontsize=title_fontsize, pad=12
            )
            from matplotlib.patches import Patch
...
def plot_acquisition_geometry(ax, sources, receivers, d, mirror=False):
    ...
    for n_s, source in enumerate(sources):
        ...
        ax.plot(src_x, src_y, 'ro', 
               markersize=markersize+3, alpha=0.7, label='Source' if n_s == 0 else None,
               zorder=50)
        ...
        for n_r, rcvr in enumerate(receivers[n_s]):
            ...
            ax.plot([src_x, rcvr_x], [src_y, rcvr_y], 
                   color='gold', linewidth=0.3, alpha=0.15, 
