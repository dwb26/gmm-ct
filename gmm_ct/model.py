...
Everything needed to run the algorithm lives in this file:
  - PeakData     – lightweight container for peak-detection results
  - GMM_reco     – forward model + 4-stage reconstruction (no mixins)
...
def NewtonRaphsonLBFGS(func, x0, *args, tol=1e-5, max_iter=100,
    ...
    ----------
    func : callable
        Function whose root is sought; returns a scalar or 1-D tensor.
    x0 : torch.Tensor
        Initial guess (``requires_grad`` is set automatically).
    *args : tuple
    ...
    optimizer = torch.optim.LBFGS(
        [x0], max_iter=max_iter, tolerance_grad=tol,
        tolerance_change=tol, line_search_fn=line_search_fn,
    )
...
# ==========================================================================
# GMM_reco – forward model + 4-stage reconstruction
# ==========================================================================
...
class GMM_reco:
    ...
    def __init__(self, d, N, sources, receivers, x0s, a0s,
        ...
        self.sources = [
            s.to(self.device) if isinstance(s, torch.Tensor)
            else torch.tensor(s, device=self.device)
            for s in sources
        ]
        self.receivers = [
            [
                r.to(self.device) if isinstance(r, torch.Tensor)
    ...
    def from_config(cls, cfg):
        """Create a ``GMM_reco`` from a :class:`~gmm_ct.config.ReconstructConfig`."""
        device = torch.device(
    ...
    def fit(self, proj_data, t):
        ...
        proj_data : list of torch.Tensor
            Observed projections (one tensor per source).
        t : torch.Tensor
            Time vector.
